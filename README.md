...
The open api specification is at **http://localhost:38190/swagger/index.html**.
...
- **MFC.Actors** is code related to actors communicating to get the scenarios processed.
...
However, having a different storage for scenarios prevent me from creating relations between the routes and the planets of the bounty hunters which could lead to invalid scenarios being accepted (scenario with an unknown planet). I added validation for this and tests to prevent this and give me some guarantee. However, in the context of a team or simply the addition of a bug in the validation it could lead to corrupt data.
...
If an itinerary has more or equal days left, more or equal autonomy and less or equal bounty hunters encounters, then it is not worth considering worst itineraries. Does it happen often enough to justify adding this check? Benchmarks could help me verify it is an improvement.
...
I created two views and some components. In the eventuality, there is more development made on this project, it would be worth considering creating a design system for this application -> more consistence and reusability.
## Backend
### Error handling
There is a lack of advanced error handling. I focused more on validation of inputs, and their tests, rather than error handling as it is quite time consuming. But in real life application, transient errors can happen (temporary network issue, so you have to handle those) and you need to guard against them. Logging is the bare minimum. Bubbling up useful informations and possible actions to fix those to the user is best.
...
To completely remedy the issue, we can think of using a dedicated system (Redis as a queue for example) to keep the queue, then several supervisors could pick up work in this queue making it more resilient.
## Some considerations about multi tenancy and security
...
- responses should only contain data the user is authorized to access.
...
HTTPS (Create and manage certificates)

Cookie management (TTL, security flags)

CORS: Do we want other domains to make calls to our API? For integration purposes for example.
